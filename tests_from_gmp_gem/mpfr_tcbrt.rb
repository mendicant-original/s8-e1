...
class MPFR_TCBRT < Test::Unit::TestCase
  ...
  def special
    ...
    assert(x.nan?, "@nan.cbrt should be NaN")
    x = @inf.cbrt(GMP::GMP_RNDN)
    assert(x.infinite?, "@inf.cbrt should be +inf")
    assert(x > 0, "@inf.cbrt should be +inf")
    x = @neg_inf.cbrt(GMP::GMP_RNDN)
    assert(x.infinite?, "@neg_inf.cbrt should be -inf")
    assert(x < 0, "@neg_inf.cbrt should be -inf")
    x = @zero.cbrt(GMP::GMP_RNDN)
    assert(x.zero?, "@zero.cbrt should be +0")
    assert_equal(GMP::F(0), x, "@zero.cbrt should be +0")
    x = @neg_zero.cbrt(GMP::GMP_RNDN)
    assert(x.zero?, "@neg_zero.cbrt should be -0")
