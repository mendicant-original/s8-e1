...
class MPFR_TREC_SQRT < Test::Unit::TestCase
  ...
  def special
    ...
    assert(x.nan?, "@nan.rec_sqrt should be NaN")
    x = @inf.rec_sqrt(GMP::GMP_RNDN)
    assert(x.zero?, "@inf.rec_sqrt should be +0")
    assert(x.hash != @neg_zero.hash, "@inf.rec_sqrt should be +0")
    x = @neg_inf.rec_sqrt(GMP::GMP_RNDN)
    assert(x.nan?, "@neg_inf.rec_sqrt should be NaN")
    x = @zero.rec_sqrt(GMP::GMP_RNDN)
    assert(x.infinite?, "@zero.rec_sqrt should be inf")
    x = @neg_zero.rec_sqrt(GMP::GMP_RNDN)
    assert(x.infinite?, "@neg_zero.rec_sqrt should be inf")
    x = @neg_one.rec_sqrt(GMP::GMP_RNDN)
    assert(x.nan?, "@neg_one.rec_sqrt should be NaN")
